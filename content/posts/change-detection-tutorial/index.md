...
*Originally published on my old blog, [Pafnuty blog](https://pafnuty.wordpress.com). Reposted here as an effort to [consolidate writing](/posts/consolidating-my-writing/) into one place. The original publication date was: May 29, 2014.*
...
{{< figure src="example_signal_01.png" alt="A toy signal for change detection" caption="A toy signal for change detection. What kind of change is interesting? What is important to detect and what should be ignored?" >}}
...
{{< figure src="online_detection_01.png" alt="online_detection_01" caption="Simulating online detection algorithms. Here the stopping condition (fired at the vertical red line) is triggered immediately after the change." >}}
{{< figure src="seasonal_signal_01.png" alt="seasonal_signal_01" caption="Detecting change in a seasonal signal (shown on top). The residual (on bottom) rises sharply after change, but is resistant to the seasonal variation." >}}
{{< figure src="noise_outliers_01.png" alt="noise_outliers_01" caption="We study a signal with outliers and noise, and try to design a detector sensitive to change using z-scores.I've been working on a number of problems in the past few years on change detection and anomaly detection." >}}
...
{{< figure src="ekg_plot_01.png" alt="EKG signal" caption="EKG signal" >}}
{{< figure src="ekg_basis_dictionary_01.png" alt="EKG_basis_dictionary_01" caption="Using clustering, we construct a dictionary of small signal windows from which to reconstruct a full signal. Here are a few of the windows in the dictionary." >}}
...
{{< figure src="reconstructed_ekg_01.png" alt="reconstructed_EKG_01" caption="A reconstruction (bottom row) of an EKG signal (middle row). The top row shows both signals superimposed." >}}
